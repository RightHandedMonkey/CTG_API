...
public class SyncNetworkDownloadCommand implements Command {
	...
	public synchronized Result execute() {
		...
		r =  sm.handleSyncTableDownload(c, CTGWebHelper.getHost(), CTGTablePool.getDbName(), ctgTable, new CTGTag(), itemsPerPage, ctagInfo.getDownloadDate());
		TagListNotifier.getNotifier().updateList();
		if (checkAbort(r)) {
			return r;
		}
		r.add_results_if_error(sm.handleSyncTableDownload(c, CTGWebHelper.getHost(), CTGTablePool.getDbName(), ctTable, new CTGChecklistTemplate(), itemsPerPage, ctInfo.getDownloadDate()), "");
		CTListNotifier.getNotifier().updateList();
		if (checkAbort(r)) {
			return r;
		}
		r.add_results_if_error(sm.handleSyncTableDownload(c, CTGWebHelper.getHost(), CTGTablePool.getDbName(), citTable, new CTGChecklistItemTemplate(), itemsPerPage, citInfo.getDownloadDate()), "");
		...
		if(NetAuthentication.isReadyForLogin()) {
			r.add_results_if_error(sm.handleSyncTableDownload(c, CTGWebHelper.getHost(), CTGTablePool.getDbName(), rcTable, new CTGRunChecklist(), itemsPerPage, rcInfo.getDownloadDate()), "");
			RCListNotifier.getNotifier().updateList();
			if (checkAbort(r)) {
				return r;
			}
			r.add_results_if_error(sm.handleSyncTableDownload(c, CTGWebHelper.getHost(), CTGTablePool.getDbName(), rciTable, new CTGRunChecklistItem(), itemsPerPage, rciInfo.getDownloadDate()), "");
