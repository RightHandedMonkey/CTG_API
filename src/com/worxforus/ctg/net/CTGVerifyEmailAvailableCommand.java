...
public class CTGVerifyEmailAvailableCommand implements Command {
	...
	public CTGVerifyEmailAvailableCommand(Context c, String emailToRegister) {
		this.c = c;
	...
	public synchronized Result execute() {
		...
		Result r = new Result();
		//store email in string so it can be passed to the next task in the sequence
		r.string = this.email;
		...
		if (NetHandler.isNetworkConnected(c)) { // check if the network is ready
			...
			NetResult netResult = NetHandler.handlePostWithRetry(CTGNetConstants.CTG_HOST+CTGNetConstants.CTG_VERIFY_EMAIL, params,
		...
		Utils.LogD(this.getClass().getName(), "Finish CTG Email check for availability - took "
